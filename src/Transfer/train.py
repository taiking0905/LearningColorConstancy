...
from config import BASE_DIR, TRAIN_DIR, VAL_DIR, TEST_DIR, REAL_RGB_JSON_PATH, EPOCHS, OUTPUT_DIR, BATCH_SIZE, LEARNING_RATE, WEIGHT, SEED, ERASE_PROB, ERASE_SIZE, DEVICE, set_seed, START_EPOCH_2, START_EPOCH_3, FREEZE_MODE
import pandas as pd
...
def main():
    ...
    print(model)
    model.load_state_dict(torch.load(OUTPUT_DIR / 'resnet-18_model.pth'))
    ...
    optimizer_step = torch.optim.Adam(freeze_layerstrainable_params, lr=LEARNING_RATE, weight_decay=WEIGHT)
    ...
    for epoch in range(EPOCHS):
        ...
        if best_metrics is None or is_better(curr_metrics, best_metrics):
            ...
            torch.save(model.state_dict(), OUTPUT_DIR / f'transfer_model.pth')
